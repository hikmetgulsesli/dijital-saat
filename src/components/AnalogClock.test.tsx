...
describe('AnalogClock', () => {
  ...
  it('renders hour, minute, and second hands', () => {
    ...
    const lines = document.querySelectorAll('line')
    // Should have 12 hour markers + 3 hands
...
describe('useClockTypePreference', () => {
  ...
  it('returns digital as default when no localStorage value', () => {
    localStorageMock.getItem.mockReturnValue(null)
    
    let result: [ClockType, (type: ClockType) => void] | undefined
    
    function TestComponent() {
      result = useClockTypePreference()
      return null
    }
    
    render(<TestComponent />)
    expect(result?.[0]).toBe('digital')
  })

  it('reads clock type from localStorage', () => {
    ...
    render(<TestComponent />)
    expect(result?.[0]).toBe('analog')
  })

  it('saves clock type to localStorage when changed', () => {
    localStorageMock.getItem.mockReturnValue('digital')
    ...
    expect(localStorageMock.setItem).toHaveBeenCalledWith('dijital-saat-clock-type', 'analog')
  })
})
